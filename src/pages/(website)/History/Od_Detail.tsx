...
const Od_Detail = () => {
    ...
    const handleCancelOrder = async () => {
        ...
        if (!token) {
            ...
            message.success(response.data.message || "Đơn hàng đã hủy thành công.");
                    ...
                    <div className="wd-form-order">
                        <div className="order-head">
                            <figure className="img-product">
                                <img
                                    src={order.order_detail[0]?.product_image}
                        ...
                        <div className="widget-tabs style-has-border widget-order-tab">
                            ...
                            <ul className="widget-menu-tab">
                                ...
                                >
                                    <span className="inner">Chi tiết đơn hàng</span>
                                </li>
                                ...
                                >
                                    <span className="inner">Lịch sử đơn hàng</span>
                                </li>
                            </ul>
                                    ...
                                    <div className="widget-content-inner active">
                                        {order.order_detail.map((item) => (
                                            ...
                                            >
                                                ...
                                                <figure className="img-product" style={{ marginRight: "15px" }}>
                                                    <img
                                                ...
                                                <div className="content">
                                                    ...
                                                    <div className="mt_4">
                                                    <span className="fw-6">Màu sắc:</span> {item.product_variant.color.name}
                                                    </div>
                                                    <div className="mt_4">
                                                    <span className="fw-6">Kích thước:</span> {item.product_variant.size.name}
                                                    </div>
                                                </div>
                                                </div>

                                                {/* Phần giá và số lượng */}
                                                <div className="product-price-quantity" style={{ textAlign: "right" }}>
                                                <div className="quantity" style={{ marginBottom: "8px", fontWeight: "bold" }}>
                                                    x{item.quantity}
                                                </div>
                                                <div className="price">
                                        ...
                                        <div className="total-price" style={{ textAlign: "right" }}>
                                            Tổng tiền: 
                                            <span style={{ color: "red",fontSize: "15px", fontWeight: "bold" }}>
                                                {new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(calculateTotalPrice(order))}
                                            </span>
                                        </div>
                                    ...
                                    <ul className="timeline">
                                        ...
                                        {Number(order.status) === 7 ? (
                                            ...
                                            <li>
                                                ...
                                                <div className="timeline-box">
                                                    <a className="timeline-panel" href="javascript:void(0);">
                                                        <div className="text-2 fw-6" style={{ color: "green" }}>
                                                            {orderStatusMap[7]}
                                                        </div>
                                            ...
                                            Object.keys(orderStatusMap).map((key) => {
                                                ...
                                                if (statusKey === 7) return null;

                                                // Ẩn trạng thái "Giao hàng thất bại" (5) trừ khi status từ backend là 5
                                                if (statusKey === 5 && currentStatus !== 5) return null;

                                                // Ẩn "Giao hàng thành công" (4) và "Hoàn thành" (6) khi có "Giao hàng thất bại" (5)
                                                if (currentStatus === 5 && (statusKey === 4 || statusKey === 6)) return null;

                                                const isCompleted = statusKey <= currentStatus;

                                                return (
                                                    <li key={statusKey}>
                                                        <div className={`timeline-badge ${isCompleted ? "success" : ""}`}></div>
                                                        <div className="timeline-box">
                                                            <a className="timeline-panel" href="javascript:void(0);">
                                                                <div
                                                                    className="text-2 fw-6"
                                                                    style={{
                                                                        color: isCompleted ? "green" : "gray", // Màu cho trạng thái đã hoàn thành
                                                                    }}
                                                                >
                                                                    {orderStatusMap[statusKey]}
                                                                </div>
                                                                {isCompleted && <span>{order.updated_at}</span>}
                                                            </a>
                                                        </div>
                                                    </li>
                                                );
